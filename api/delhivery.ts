      ...
      if (action === 'diagnostic') {
        ...
        if (!data.success || data.error === true || !data.packages || data.packages.length === 0 || !data.packages[0].waybill || data.packages[0].status === "Fail") {
          ...
          if (errorMsg.includes('ClientWarehouse')) {
            ...
            errorMsg = `Delhivery pickup warehouse ('${usedWarehouse}') is not registered or active. Please check the exact warehouse name configured in your Delhivery account.Actual Error: ${errorMsg}`;
          }
