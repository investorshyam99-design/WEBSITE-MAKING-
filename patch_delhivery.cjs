...
let code = fs.readFileSync('api/delhivery.ts', 'utf8');

const search = `      const data: any = await response.json();
...
const replace = `      const data: any = await response.json();
      if (!data.success && (!data.packages || data.packages.length === 0 || !data.packages[0].waybill)) {
...
fs.writeFileSync('api/delhivery.ts', code);
console.log('patched delhivery error handling');
