          ...
          {TABS.map(tab => (
            <button 
              ...
              className={`px-4 py-3 whitespace-nowrap text-xs font-bold uppercase tracking-wider transition-colors border-b-2 flex-shrink-0 ${
                activeTab === tab.id ? 'border-[#1E2A44] text-[#1E2A44]' : 'border-transparent text-gray-400 hover:text-gray-600'
              }`}
            >
              {tab.label}
              {tab.id === 'new' && newOrders.length > 0 && <span className="ml-2 bg-blue-100 text-blue-800 py-0.5 px-2 rounded-full text-[10px]">{newOrders.length}</span>}
              {tab.id === 'drafts' && draftOrders.length > 0 && <span className="ml-2 bg-amber-100 text-amber-800 py-0.5 px-2 rounded-full text-[10px]">{draftOrders.length}</span>}
            </button>
          ))}
        </div>
      </div>
      ...
      {isExpanded && (
        <div className="border-t border-gray-100 bg-gray-50/50 p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs bg-white p-3 rounded-lg border border-gray-100">
            ...
            <div className="col-span-2">
              <p className="text-gray-400 font-bold uppercase tracking-wider mb-1">Phone</p>
              <div className="flex items-center justify-between bg-gray-50 px-2 py-1.5 rounded">
                <span className="font-semibold text-gray-800">{order.phone || 'No Phone provided'}</span>
                {order.phone && (
                  <button onClick={(e) => handleCopy(e, order.phone!)} className="text-gray-400 hover:text-gray-600">
            ...
            {order.address && (
               <div className="col-span-2">
                 <p className="text-gray-400 font-bold uppercase tracking-wider mb-1">Address</p>
                 <div className="flex items-start justify-between bg-gray-50 px-2 py-1.5 rounded gap-2">
                   <span className="font-semibold text-gray-800 leading-tight">{order.address}</span>
                   <button onClick={(e) => handleCopy(e, order.address!)} className="text-gray-400 hover:text-gray-600 mt-0.5">
                     <Copy className="h-3 w-3" />
                   </button>
                 </div>
               </div>
          ...
          <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                       ...
                       <button 
                         onClick={() => {
                           ...
                           window.open(generateWhatsAppLink(order.phone || '', templates.shipped), '_blank');
            ...
            {activeTab === 'abandoned' && (
              <>
                <button 
                  onClick={(e) => handleWhatsApp(e, "Your Jersey Unicorn cart is waiting ⚽")}
                  className="w-full py-2.5 bg-[#25D366] text-white text-xs font-bold uppercase tracking-wider rounded-lg flex items-center justify-center gap-2 shadow-sm"
                >
