...
(async () => {
  ...
  page.on('request', request => {
    if (request.url().includes('checkout.razorpay.com/v1/checkout.js')) {
      console.log("Intercepted Razorpay JS");
      request.respond({
        status: 200,
  ...
  console.log("Navigating to home to add item to cart");
  await page.goto('http://localhost:3000', { waitUntil: 'networkidle0' });

  // Evaluate directly to set up cart and user
  await page.evaluate(() => {
  ...
  // Need to fill pincode to pass DeliveryChecker
  ...
  // Wait for the handler to be called and the potential crash
